"""CredPilot's HTTP API: the endpoints, and nothing that renders a page.
...
``GET  /api/mcp``
    The surface the MCP server advertises.

Exposed as an :class:`~fastapi.APIRouter` rather than an application, so it can
be mounted into one that serves a UI (:mod:`src.web`) or into one that does
not. The page and the static assets live in :mod:`src.web`; nothing here knows
they exist.
...
response, and :func:`_public` drops the internal state the caller has no
business seeing — the raw packet above all, which carries the applicant's
address and masked identifiers and is not needed to render an answer.
"""
...
#: The router every CredPilot HTTP surface mounts.
router = APIRouter()
